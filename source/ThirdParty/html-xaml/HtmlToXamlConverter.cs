...
namespace HTMLConverter
{
    ...
    public static class HtmlToXamlConverter
    {
        ...
        public static Section DeserializeSection(string strFlowDocumentXML)
        {
            Debug.WriteLine("DeserializeSection: " + strFlowDocumentXML);
            try
            {
                string temp = strFlowDocumentXML;
                if (temp.StartsWith("<FlowDocument"))
                {
                ...
                using (MemoryStream xamlMemoryStream = new MemoryStream(Encoding.ASCII.GetBytes(temp)))
                {
            ...
            for (XmlNode htmlNode = htmlElement.FirstChild; htmlNode != null; htmlNode = htmlNode.NextSibling)
            {
                if (htmlNode is XmlElement)
                {
                    ...
                    if (HtmlSchema.IsInlineElement(htmlChildName) || HtmlSchema.IsBlockElement(htmlChildName) || 
                        ...
                        htmlChildName == "sub" || htmlChildName == "sup")
                    {
        ...
        private static void AddImage(XmlElement xamlParentElement, XmlElement htmlElement, Hashtable inheritedProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            xmlAttrib = htmlElement.Attributes["alt"];
            if (null != xmlAttrib)
            {
                xamlImageElement.SetAttribute("Tag", xmlAttrib.Value);
            }

            xmlAttrib = htmlElement.Attributes["STYLE"];
            if (null != xmlAttrib)
            {
                CssStylesheet css = new CssStylesheet(htmlElement);
                Hashtable localProperties = new Hashtable();
                List<XmlElement> sourceContent = new List<XmlElement>();
                sourceContent.Add(htmlElement);
                HtmlCssParser.GetElementPropertiesFromCssAttributes(htmlElement, "IMG", css, localProperties, sourceContent);

                foreach (string key in localProperties.Keys)
                {
                    switch (key)
                    {
                        case "width":
                            xamlImageElement.SetAttribute("Width", localProperties[key] as string);
                            break;
                        case "height":
                            xamlImageElement.SetAttribute("Height", localProperties[key] as string);
                            break;
                    }
                }
            }
        ...
        private static void AddColumnInformation(XmlElement htmlTableElement, XmlElement xamlTableElement, ArrayList columnStartsAllRows, Hashtable currentProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            {
                XmlElement xamlTableColumns = xamlTableElement.OwnerDocument.CreateElement(null, Xaml_Columns, _xamlNamespace);
                xamlTableElement.AppendChild(xamlTableColumns);
                ...
                for (int columnIndex = 0; columnIndex < columnStartsAllRows.Count - 1; columnIndex++)
                {
                    ...
                    xamlTableColumns.AppendChild(xamlColumnElement);
                }
            }
            else
            {
            ...
            while (propertyEnumerator.MoveNext())
            {
                switch ((string)propertyEnumerator.Key)
                {
                    ...
                    case "BaselineAlignment":
                        xamlElement.SetAttribute("BaselineAlignment", propertyEnumerator.Value as string);
                        break;
                }
            }
        ...
        private static Hashtable GetElementProperties(XmlElement htmlElement, Hashtable inheritedProperties, out Hashtable localProperties, CssStylesheet stylesheet, List<XmlElement> sourceContext)
        {
            ...
            switch (elementName)
            {
                ...
                case "samp":
                    ...
                    break;
                case "sub":
                    localProperties["BaselineAlignment"] = "Subscript";
                    break;
                case "sup":
                    localProperties["BaselineAlignment"] = "Superscript";
                    break;
        ...
        public const string Xaml_Columns = "Table.Columns";
