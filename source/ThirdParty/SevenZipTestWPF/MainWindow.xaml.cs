...
using Lenovo.OneKeyBackup.Common;
    ...
    public partial class MainWindow : Window
    {
        ...
        private void b_Archive_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == CommonFileDialogResult.OK)
            {
                ...
                SevenZipExtractor.SetLibraryPath(Path.Combine(folder, "ThirdParty\\7z\\7z.dll"));
        ...
        private void b_Extract_Click(object sender, RoutedEventArgs e)
        {
            string folder = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            SevenZipExtractor.SetLibraryPath(Path.Combine(folder, "ThirdParty\\7z\\7z.dll"));
       //     SevenZipExtractor.SetLibraryPath(Path.Combine(Path.GetDirectoryName(current.Location), "7z.dll"));
            tb_Messages.Text = "Started" + Environment.NewLine;
            string fileName = tb_ExtractArchive.Text;
            string directory = tb_ExtractFolder.Text;
            var extractor = new SevenZipExtractor(fileName, "1");
        ...
        {
            string folder = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            SevenZipCompressor.SetLibraryPath(Path.Combine(folder, "ThirdParty\\7z\\7z.dll"));
            ...
            cmp.BeginCompressDirectory(directory, archFileName, "1");  
        }
        ...
        {
            this.taskListView.ItemsSource = TaskManager.Instance.BackupTaskCollection;
        }

        private void pauseButton_Click(object sender, RoutedEventArgs e)
        {
            if (this.taskListView.SelectedItem == null)
                return;

            Task task = this.taskListView.SelectedItem as Task;
            if (task != null)
