...
{
    public class InfiniteDecimalDataCreator : DataCreator
    {
        private static InfiniteDecimalDataCreator creator;

        public static InfiniteDecimalDataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new InfiniteDecimalDataCreator();
        ...
        {
            this.exerciseTitle = "无限小数练习";
            this.examTitle = "无限小数测验";
            this.flowDocumentFile = "SoonLearning.Math.Decimal_InfiniteDecimal.InfiniteDecimalDocument.xaml";
        ...
        private void CreateMCQuestion(SectionBaseInfo sectionInfo, Section section)
        {
            ...
            string questionText = string.Format("选出是无限小数的数。");
            ...
            foreach (QuestionOption option in mcQuestion.QuestionOptionCollection)
            {
                ...
                if (content.QuestionPartCollection[0] is QuestionTextPart)
                {
                    ...
                    {
                        strBuilder.AppendLine(string.Format("{0}是无限小数，是正确答案。", decimalPart.Text));
                    }
                }
            }
        ...
        private void CreateTableQuestion(SectionBaseInfo sectionInfo, Section section)
        {
            ...
            string questionText = string.Format("请下表中选出所有的无限小数。");
        ...
        public InfiniteDecimalDataCreator()
        {
