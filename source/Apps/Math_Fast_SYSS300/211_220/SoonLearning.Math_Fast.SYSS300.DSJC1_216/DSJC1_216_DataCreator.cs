...
{
    public class DSJC1_216DataCreator : DataCreator
    {
        private static DSJC1_216DataCreator creator;

        public static DSJC1_216DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new DSJC1_216DataCreator();
        ...
        {
            this.exerciseTitle = "定身减除法（一）练习";
            this.examTitle = "定身减除法（一）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.DSJC1_216.DSJC1_216_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int a1 = 1;// rand.Next(0, 5); //百位数
            int b1 = rand.Next(0, 3); //十位数
            int c1 = rand.Next(1, 6); //个位数
            ...
            if (posRand == 0)
            {
                A1 = (rand.Next(baseNum * 100 + baseNum, maxValue + 1) / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
