...
namespace SoonLearning.Math_Fast.SYSS300.CZJC1_218
{
    public class CZJC1_218DataCreator : DataCreator
    {
        private static CZJC1_218DataCreator creator;

        public static CZJC1_218DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new CZJC1_218DataCreator();
        ...
        {
            this.exerciseTitle = "凑整除法（一）练习";
            this.examTitle = "凑整除法（一）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.CZJC1_218.CZJC1_218_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int b1 = 9;// rand.Next(0, 3); //十位数
            int c1 = rand.Next(5, 10); //个位数
            ...
            int baseNum = a1 * 100 + b1 * 10 + c1;
            valueABC.baseNum = n;// a1 * 100 + b1 * 10 + c1;
            
            if (posRand == 0)
            {
                A1 = (rand.Next(baseNum + baseNum, maxValue + 1) / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
