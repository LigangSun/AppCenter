...
{
    public class JDBS1_153DataCreator : DataCreator
    {
        private static JDBS1_153DataCreator creator;

        public static JDBS1_153DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new JDBS1_153DataCreator();
        ...
        {
            this.exerciseTitle = "9的倍数法（一）练习";
            this.examTitle = "9的倍数法（一）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.JDBS1_153.JDBS1_153_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                ...
                {
                    A2 = a2 * 1000 + 9 * 100 + 9 * 10 + c2;
                }
                else if (n == 3)
                {
                    A2 = a2 * 10000 + 9 * 1000 + 9 * 100 + 9 * 10 + c2;
                }
            }
            else
            {
