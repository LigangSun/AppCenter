...
namespace SoonLearning.Math_Fast.SYSS300.QMDJ_151
{
    public class QMDJ_151DataCreator : DataCreator
    {
        private static QMDJ_151DataCreator creator;

        public static QMDJ_151DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new QMDJ_151DataCreator();
        ...
        {
            this.exerciseTitle = "求幂的积法练习";
            this.examTitle = "求幂的积法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.QMDJ_151.QMDJ_151_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A1 = (decimal)System.Math.Pow(2, n);//rand.Next(minValue, maxValue + 1); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
