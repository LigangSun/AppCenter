...
{
    public class EWSTC_152DataCreator : DataCreator
    {
        private static EWSTC_152DataCreator creator;

        public static EWSTC_152DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new EWSTC_152DataCreator();
        ...
        {
            this.exerciseTitle = "二位数通乘法练习";
            this.examTitle = "二位数通乘法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.EWSTC_152.EWSTC_152_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                ...
                A2 = rand.Next(minValue, maxValue); //1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
                A2 = rand.Next(minValue, maxValue); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = rand.Next(minValue, maxValue); //1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
