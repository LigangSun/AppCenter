...
{
    public class YCDJ2_147DataCreator : DataCreator
    {
        private static YCDJ2_147DataCreator creator;

        public static YCDJ2_147DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new YCDJ2_147DataCreator();
        ...
        {
            this.exerciseTitle = "依次得积法（二）练习";
            this.examTitle = "依次得积法（二）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.YCDJ2_147.YCDJ2_147_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A1 = rand.Next(100, 1000); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = 101;// rand.Next(1, 10) * 9; ;// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
                A2 = rand.Next(100, 1000); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
