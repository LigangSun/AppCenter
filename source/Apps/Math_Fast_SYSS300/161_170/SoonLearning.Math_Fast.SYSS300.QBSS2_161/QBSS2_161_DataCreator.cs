...
{
    public class QBSS2_161DataCreator : DataCreator
    {
        private static QBSS2_161DataCreator creator;

        public static QBSS2_161DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new QBSS2_161DataCreator();
        ...
        {
            this.exerciseTitle = "7倍速算法（二）练习";
            this.examTitle = "7倍速算法（二）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.QBSS2_161.QBSS2_161_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A1 = 14 * (rand.Next(minValue, maxValue + 1) / 10) + rand.Next(1, 14); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = 7;// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
        ...
        private String SolveSteps(valuesStruct valueMC)
        {
            ...
            calSteps += "+";
            calSteps += a7.ToString();

            //第三步 
            int n2100 = n100 - n2;
            calSteps += "=";
            calSteps += n2100.ToString();
            calSteps += "+";
            calSteps += a7.ToString();
