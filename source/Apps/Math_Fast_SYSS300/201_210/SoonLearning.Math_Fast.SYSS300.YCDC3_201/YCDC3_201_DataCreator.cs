...
{
    public class YCDC3_201DataCreator : DataCreator
    {
        private static YCDC3_201DataCreator creator;

        public static YCDC3_201DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new YCDC3_201DataCreator();
        ...
        {
            this.exerciseTitle = "以乘代除法（三）练习";
            this.examTitle = "以乘代除法（三）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.YCDC3_201.YCDC3_201_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                ...
                A2 = 25;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
