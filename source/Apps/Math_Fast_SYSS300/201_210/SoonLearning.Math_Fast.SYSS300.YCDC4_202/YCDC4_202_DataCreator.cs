...
{
    public class YCDC4_202DataCreator : DataCreator
    {
        private static YCDC4_202DataCreator creator;

        public static YCDC4_202DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new YCDC4_202DataCreator();
        ...
        {
            this.exerciseTitle = "以乘代除法（四）练习";
            this.examTitle = "以乘代除法（四）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.YCDC4_202.YCDC4_202_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A1 = (rand.Next(minValue, maxValue + 1));// / 25) * 25; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = 0.25M;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
