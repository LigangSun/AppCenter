...
namespace SoonLearning.Math_Fast.SYSS300.YCDC10_208
{
    public class YCDC10_208DataCreator : DataCreator
    {
        private static YCDC10_208DataCreator creator;

        public static YCDC10_208DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new YCDC10_208DataCreator();
        ...
        {
            this.exerciseTitle = "以乘代除法（十）练习";
            this.examTitle = "以乘代除法（十）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.YCDC10_208.YCDC10_208_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int n = rand.Next(1, 7);
            valueABC.baseNum = n;
            decimal baseNum = 0.03125M;

            if (posRand == 0)
            {
                A1 = (rand.Next(minValue, maxValue + 1));// / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
