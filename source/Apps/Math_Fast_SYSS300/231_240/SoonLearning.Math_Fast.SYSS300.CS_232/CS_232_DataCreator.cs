...
{
    public class CS_232DataCreator : DataCreator
    {
        private static CS_232DataCreator creator;

        public static CS_232DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new CS_232DataCreator();
        ...
        {
            this.exerciseTitle = "凑商法练习";
            this.examTitle = "凑商法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.CS_232.CS_232_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                ...
                int tmp = rand.Next(10, 100); ;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
                A1 = A2 * (tmp + rand.Next(1, 4));
                valueABC.values[0] = A1;
                valueABC.values[1] = A2;
                valueABC.answer = A1 / (A2);
                valueABC.baseNum = tmp;
            }
            else
            {
                A2 = rand.Next(10, 100); ;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
