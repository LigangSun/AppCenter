...
{
    public class SJJMDataCreator : DataCreator
    {
        private static SJJMDataCreator creator;

        public static SJJMDataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new SJJMDataCreator();
        ...
        {
            this.exerciseTitle = "身加减满法练习";
            this.examTitle = "身加减满法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.SJJM.SJJM_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            while (true)
            {
                ...
                int tmpMaxValue2 = decimal.ToInt32(ManShu) - tmpMaxValue1;

                if (tmpMaxValue1 > tmpMaxValue2)
                {
                    A = tmpMaxValue1;
                    B = tmpMaxValue2;
                    break;
                }
                else if (tmpMaxValue1 < tmpMaxValue2)
                {
                    A = tmpMaxValue2;
                    B = tmpMaxValue1;
                    break;
