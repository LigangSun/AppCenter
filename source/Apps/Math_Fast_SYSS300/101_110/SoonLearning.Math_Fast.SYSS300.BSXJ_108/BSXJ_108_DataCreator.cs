...
{
    public class BSXJ_108DataCreator : DataCreator
    {
        private static BSXJ_108DataCreator creator;

        public static BSXJ_108DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new BSXJ_108DataCreator();
        ...
        {
            this.exerciseTitle = "倍数相加法练习";
            this.examTitle = "倍数相加法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.BSXJ_108.BSXJ_108_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int b2 = rand.Next(1, 5); //十位数
            int times = rand.Next(1, 5);
            int c2 = b2 * times;// rand.Next(1, b2); //个位数
            while (true)
            {
                if (c2 < 10)
                {
