...
{
    public class QHSC_229DataCreator : DataCreator
    {
        private static QHSC_229DataCreator creator;

        public static QHSC_229DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new QHSC_229DataCreator();
        ...
        {
            this.exerciseTitle = "去号速除法练习";
            this.examTitle = "去号速除法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.QHSC_229.QHSC_229_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                ...
                A3 = rand.Next(1, maxValue / 10);// / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = baseNum * rand.Next(1, maxValue / 10); ;
            ...
            else
            {
                ...
                int randA2 = rand.Next(1, 10) * 10 - randA;
                A3 = rand.Next(1, maxValue / 10);// / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = baseNum * rand.Next(1, maxValue / 10) * A3 * rand.Next(1, maxValue / 10);
                valueABC.values[0] = A1;
                valueABC.values[1] = A2;
                valueABC.values[2] = A3;
                valueABC.values[3] = A4;
