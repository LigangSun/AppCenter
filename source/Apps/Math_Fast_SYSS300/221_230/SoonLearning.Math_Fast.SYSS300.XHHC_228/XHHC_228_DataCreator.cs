...
{
    public class XHHC_228DataCreator : DataCreator
    {
        private static XHHC_228DataCreator creator;

        public static XHHC_228DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new XHHC_228DataCreator();
        ...
        {
            this.exerciseTitle = "先和后除法练习";
            this.examTitle = "先和后除法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.XHHC_228.XHHC_228_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int baseNum = rand.Next(minValue + 1, maxValue / 10);// a1 * 100 + b1 * 10 + c1;
            valueABC.baseNum = n;// a1 * 100 + b1 * 10 + c1;
            
            if (posRand == 0)
            {
                A4 = baseNum;// rand.Next(minValue + 1, maxValue);// / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = baseNum;//A4 * rand.Next(1, 12); ;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
                int randA = rand.Next(2, 10);
                int randA2 = rand.Next(1, 10) * 10 - randA;
                A3 = baseNum * randA;// / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = baseNum * randA2;
            ...
            else
            {
                A4 = baseNum;// rand.Next(minValue + 1, maxValue);// / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = baseNum;//A4 * rand.Next(1, 12); ;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
                int randA = rand.Next(2, 15);
                int randA2 = randA + rand.Next(1, 10) * 10;
                A3 = baseNum * randA;// / baseNum) * baseNum; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = baseNum * randA2;
                valueABC.values[0] = A1;
                valueABC.values[1] = A2;
        ...
        private String QuestionText(valuesStruct valueMC)
        {
            ...
            questionText += "("; questionText += valueMC.values[2].ToString();
            questionText += "÷";
