...
{
    public class LSQH1DataCreator : DataCreator
    {
        private static LSQH1DataCreator creator;

        public static LSQH1DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new LSQH1DataCreator();
        ...
        {
            this.exerciseTitle = "连数求和法(一)练习";
            this.examTitle = "连数求和法(一)测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.LSQH1.LSQH1_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            valueABC.number = rand.Next(7, 15 + 1);
            ...
            valueABC.valueOffset = rand.Next(1, 20 + 1); //等差值     
  
            for (i = 1; i < valueABC.number; i++)
            {   
                if ((i - 1) * 2 + 1 == valueABC.number) //中间值
                {
                    valueABC.valueMedian = valueABC.values[i - 1];
                }
