...
{
    public class XCHH_139DataCreator : DataCreator
    {
        private static XCHH_139DataCreator creator;

        public static XCHH_139DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new XCHH_139DataCreator();
        ...
        {
            this.exerciseTitle = "先乘后和法练习";
            this.examTitle = "先乘后和法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.XCHH_139.XCHH_139_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int a1 = rand.Next(0, 3); //百位数
            int b1 = 2;// rand.Next(0, 4); //十位数
            int c1 = 5;// rand.Next(1, 10); //个位数
            int a2 = rand.Next(0, 2); //百位数
            int b2 = 2;// rand.Next(0, 10); //十位数
            int c2 = 5;// rand.Next(0, 10); //个位数
            int d1 = 0;
            int d2 = 0;
            int a3 = 0;// rand.Next(0, 2); //百位数
            int b3 = 0; ;// rand.Next(0, 10); //十位数
            int c3 = 4;// rand.Next(0, 10); //个位数
            int a4 = 0;// rand.Next(0, 2); //百位数
            int b5 = 0;// rand.Next(0, 10); //十位数
            int c5 = rand.Next(1, 3); //个位数
            int n = rand.Next(1, 2) * 10;
            int m = rand.Next(1, 3) * 4;
            ...
            {
                A3 = 1000 * d2 + 100 * rand.Next(0, 2) + 10 * b2 + c2;
                A1 = rand.Next(1, 3) * 4;
                if (A1 == 4)
                {
                    A2 = 8;// rand.Next(1, 3) * 4;
                }
                else
                {
                    A2 = 4;
        ...
        private String SolveSteps(valuesStruct valueMC)
        {
            ...
            int A3 = decimal.ToInt32(valueMC.values[2]);
            ...
            int A2A3 = A2 * A3;
            calSteps += "=";
            calSteps += A1A3.ToString();
            calSteps += "+";
            calSteps += A2A3.ToString();
        ...
        private String QuestionText(valuesStruct valueMC)
        {
            ...
            decimal A3 = valueMC.values[2];

            //if (valueMC.posMul == 0)
            {
                questionText += "(";
                questionText += A1.ToString();
                questionText += "+";
                questionText += A2.ToString();
                questionText += ")×";
                questionText += A3.ToString();
            }
            //else
            //{
            //    questionText += A2.ToString();
