...
{
    public class HBKZ_137DataCreator : DataCreator
    {
        private static HBKZ_137DataCreator creator;

        public static HBKZ_137DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new HBKZ_137DataCreator();
        ...
        {
            this.exerciseTitle = "合并扩整法练习";
            this.examTitle = "合并扩整法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.HBKZ_137.HBKZ_137_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int[] mul1 = new int[4]{20, 25, 50, 125};
            int[] mul2 = new int[4]{5 , 4 , 2 , 8  };

            for (int i = 0; i < valueABC.number / 2; i++)
            {
                int pos = rand.Next(0, 4);
                valueABC.values[i] = mul1[pos];
                pos = rand.Next(0, 4);
                valueABC.values[i + 1] = mul2[pos];
            }
