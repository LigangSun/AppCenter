...
{
    public class XHHC_138DataCreator : DataCreator
    {
        private static XHHC_138DataCreator creator;

        public static XHHC_138DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new XHHC_138DataCreator();
        ...
        {
            this.exerciseTitle = "先和后乘法练习";
            this.examTitle = "先和后乘法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.XHHC_138.XHHC_138_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int a1 = 1;// rand.Next(1, 9); //百位数
            int b1 = rand.Next(0, 4); //十位数
            int c1 = rand.Next(1, 10); //个位数
            int a2 = 0;// rand.Next(1, 10); //百位数
            int b2 = rand.Next(0, 10); //十位数
            int c2 = rand.Next(0, 10); //个位数
            int d1 = 0;
            int d2 = 0;
            ...
            if (posRand == 0)
            {
                A1 = rand.Next(minValue, maxValue + 1); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = rand.Next(11, 100);
                A3 = 100 - A2;
                valueABC.values[0] = A1;
                valueABC.values[1] = A2;
                valueABC.values[2] = A3;
                valueABC.answer = A1 * A2 + A1 * A3;
            }
            else
            {
                A3 = rand.Next(minValue, maxValue + 1); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = rand.Next(11, 100);
                A2 = 100 - A1;
                valueABC.values[0] = A1;
                valueABC.values[1] = A2;
                valueABC.values[2] = A3;
                valueABC.answer = A3 * A2 + A3 * A1;
            }            
        }
        ...
        private String SolveSteps(valuesStruct valueMC)
        {
            ...
            else
            {
                ...
                calSteps += "×";
                calSteps += A3.ToString();
                A1 = decimal.ToInt32(valueMC.values[2]);
                A2 = decimal.ToInt32(valueMC.values[1]);
                A3 = decimal.ToInt32(valueMC.values[0]);
            }
