...
namespace SoonLearning.Math_Fast.SYSS300.BSJC_130
{
    public class BSJC_130DataCreator : DataCreator
    {
        private static BSJC_130DataCreator creator;

        public static BSJC_130DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new BSJC_130DataCreator();
        ...
        {
            this.exerciseTitle = "补数减乘法练习";
            this.examTitle = "补数减乘法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.BSJC_130.BSJC_130_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int a1 = rand.Next(1, 9); //百位数
            int b1 = rand.Next(0, 6); //十位数
            int c1 = rand.Next(0, 6); //个位数
        ...
        private String SolveSteps(valuesStruct valueMC)
        {
            ...
            calSteps += "×100-";
            calSteps += a100.ToString();
            calSteps += "×";
            calSteps += bs.ToString();
            calSteps += "-";
            calSteps += b10.ToString();
            calSteps += "×";
            calSteps += bs.ToString();
            calSteps += "-";
            ...
            calSteps += A100.ToString();
            calSteps += "-";
            calSteps += a100bs.ToString();
            calSteps += "-";                
            calSteps += b10bs.ToString();
            calSteps += "-";
            calSteps += c1bs.ToString();

            //第三步 
            decimal AA11 = A100 - a100bs - b10bs;
            decimal cc11 = c1bs;
            calSteps += "=";
