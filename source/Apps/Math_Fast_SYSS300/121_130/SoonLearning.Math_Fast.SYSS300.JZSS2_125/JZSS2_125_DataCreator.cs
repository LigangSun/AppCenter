...
namespace SoonLearning.Math_Fast.SYSS300.JZSS2_125
{
    public class JZSS2_125DataCreator : DataCreator
    {
        private static JZSS2_125DataCreator creator;

        public static JZSS2_125DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new JZSS2_125DataCreator();
        ...
        {
            this.exerciseTitle = "近整速算法（二）练习";
            this.examTitle = "近整速算法（二）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.JZSS2_125.JZSS2_125_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int a1 = 9;// rand.Next(1, 10); //百位数
            int b1 = rand.Next(5, 10); //十位数
            int c1 = rand.Next(0, 10); //个位数
            int a2 = 9;// rand.Next(1, 10); //百位数
            int b2 = rand.Next(9, 10); //十位数
