...
{
    public class JJCZ3_121DataCreator : DataCreator
    {
        private static JJCZ3_121DataCreator creator;

        public static JJCZ3_121DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new JJCZ3_121DataCreator();
        ...
        {
            this.exerciseTitle = "加减乘整法（三）练习";
            this.examTitle = "加减乘整法（三）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.JJCZ3_121.JJCZ3_121_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            decimal A1 = rand.Next(minValue, maxValue + 1);
            int baseNum = rand.Next(1, 10);
            if (bitNum == 3)
            {
                baseNum *= 100;
            }
            ...
            int error1 = rand.Next(1, 8);
            int error2 = rand.Next(1, 8);
            decimal A2 = 0;
            int sign = 0;// rand.Next(0, 2);
            if (sign == 0)
            {
                A1 = baseNum + error1;
                A2 = baseNum - error2;
            ...
            valueABC.errors[0] = error1;
            valueABC.errors[1] = error2;
            valueABC.baseNum = baseNum;
            valueABC.signs[0] = sign;
        ...
        private String SolveSteps(valuesStruct valueMC)
        {
            ...
            //解题步骤
            decimal A1 = valueMC.values[0];
            decimal A2 = valueMC.values[1];
            int a2 = decimal.ToInt32(A2);
            int baseNum = valueMC.baseNum;

            int sign = valueMC.signs[0];
            decimal error1 = valueMC.errors[0];
            decimal error2 = valueMC.errors[1];             
            ...
            //第二步 
            decimal A1A2base = A1 + A2 - baseNum;
            decimal e1 = A1 - baseNum;
            decimal e2 = baseNum - A2;
            calSteps += "=";
            calSteps += A1A2base.ToString();
            calSteps += "×";
            calSteps += baseNum.ToString();
            calSteps += "-";
            calSteps += e1.ToString();
            calSteps += "×";
            calSteps += e2.ToString();          
            
            //第三步 
            decimal Abase = A1A2base * baseNum;
            decimal e1e2 = e1 * e2;
            calSteps += "=";
            calSteps += Abase.ToString();
            calSteps += "-";         
            calSteps += e1e2.ToString();
