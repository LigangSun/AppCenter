...
{
    public class LBSS2_189DataCreator : DataCreator
    {
        private static LBSS2_189DataCreator creator;

        public static LBSS2_189DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new LBSS2_189DataCreator();
        ...
        {
            this.exerciseTitle = "625倍速算法（二）练习";
            this.examTitle = "625倍速算法（二）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.LBSS2_189.LBSS2_189_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A2 = (rand.Next(minValue, maxValue + 1) / 16) * 16; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
