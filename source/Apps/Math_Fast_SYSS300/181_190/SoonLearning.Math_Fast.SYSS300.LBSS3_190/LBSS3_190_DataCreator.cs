...
{
    public class LBSS3_190DataCreator : DataCreator
    {
        private static LBSS3_190DataCreator creator;

        public static LBSS3_190DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new LBSS3_190DataCreator();
        ...
        {
            this.exerciseTitle = "625倍速算法（三）练习";
            this.examTitle = "625倍速算法（三）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.LBSS3_190.LBSS3_190_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            int b1 = rand.Next(1, 5); //十位数
            int c1 = rand.Next(1, 5); //个位数
            ...
            if (posRand == 0)
            {
                A2 = (decimal)System.Math.Pow(2, b1) * 10 + (decimal)System.Math.Pow(2, c1);// (rand.Next(minValue, maxValue + 1) / 16) * 16; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = 625;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
