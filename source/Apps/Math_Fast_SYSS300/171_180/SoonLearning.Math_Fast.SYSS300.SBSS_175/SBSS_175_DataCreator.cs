...
{
    public class SBSS_175DataCreator : DataCreator
    {
        private static SBSS_175DataCreator creator;

        public static SBSS_175DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new SBSS_175DataCreator();
        ...
        {
            this.exerciseTitle = "37倍速算法练习";
            this.examTitle = "37倍速算法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.SBSS_175.SBSS_175_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A2 = (rand.Next(1, 9 + 1)) * 3; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = 37;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
