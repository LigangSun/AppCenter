...
namespace SoonLearning.Math_Fast.SYSS300.QBSS_180
{
    public class QBSS_180DataCreator : DataCreator
    {
        private static QBSS_180DataCreator creator;

        public static QBSS_180DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new QBSS_180DataCreator();
        ...
        {
            this.exerciseTitle = "76倍速算法练习";
            this.examTitle = "76倍速算法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.QBSS_180.QBSS_180_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A2 = (rand.Next(1, 9 + 1)) * 4; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = 76;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
