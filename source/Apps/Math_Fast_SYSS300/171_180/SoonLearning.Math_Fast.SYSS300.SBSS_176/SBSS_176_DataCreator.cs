...
{
    public class SBSS_176DataCreator : DataCreator
    {
        private static SBSS_176DataCreator creator;

        public static SBSS_176DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new SBSS_176DataCreator();
        ...
        {
            this.exerciseTitle = "42倍速算法练习";
            this.examTitle = "42倍速算法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.SBSS_176.SBSS_176_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A2 = (rand.Next(minValue, maxValue + 1)) / 5 * 5; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = 42;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
