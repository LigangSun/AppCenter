...
{
    public class SBSS_173DataCreator : DataCreator
    {
        private static SBSS_173DataCreator creator;

        public static SBSS_173DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new SBSS_173DataCreator();
        ...
        {
            this.exerciseTitle = "33倍速算法练习";
            this.examTitle = "33倍速算法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.SBSS_173.SBSS_173_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A1 = (rand.Next(minValue, maxValue + 1)) / 3 * 3 + rand.Next(1, 3); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = 33;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
