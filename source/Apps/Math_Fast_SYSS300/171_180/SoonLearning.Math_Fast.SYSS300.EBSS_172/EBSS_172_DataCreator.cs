...
{
    public class EBSS_172DataCreator : DataCreator
    {
        private static EBSS_172DataCreator creator;

        public static EBSS_172DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new EBSS_172DataCreator();
        ...
        {
            this.exerciseTitle = "26倍速算法练习";
            this.examTitle = "26倍速算法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.EBSS_172.EBSS_172_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A2 = 4 * (rand.Next(minValue, maxValue + 1) / 4); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = 26;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
