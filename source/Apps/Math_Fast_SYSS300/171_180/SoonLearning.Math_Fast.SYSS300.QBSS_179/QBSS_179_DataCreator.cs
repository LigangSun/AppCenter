...
{
    public class QBSS_179DataCreator : DataCreator
    {
        private static QBSS_179DataCreator creator;

        public static QBSS_179DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new QBSS_179DataCreator();
        ...
        {
            this.exerciseTitle = "75倍速算法练习";
            this.examTitle = "75倍速算法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.QBSS_179.QBSS_179_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A2 = (rand.Next(minValue, maxValue + 1)) / 4 * 4; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = 75;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
