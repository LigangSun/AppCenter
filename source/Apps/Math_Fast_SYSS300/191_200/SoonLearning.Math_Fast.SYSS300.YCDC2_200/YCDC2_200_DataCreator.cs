...
{
    public class YCDC2_200DataCreator : DataCreator
    {
        private static YCDC2_200DataCreator creator;

        public static YCDC2_200DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new YCDC2_200DataCreator();
        ...
        {
            this.exerciseTitle = "以乘代除法（二）练习";
            this.examTitle = "以乘代除法（二）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.YCDC2_200.YCDC2_200_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A1 = (rand.Next(minValue, maxValue + 1));// / 5) * 5; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = 0.5M;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
