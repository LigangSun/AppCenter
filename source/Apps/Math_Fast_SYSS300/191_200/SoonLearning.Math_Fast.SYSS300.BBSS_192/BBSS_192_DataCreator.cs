...
{
    public class BBSS_192DataCreator : DataCreator
    {
        private static BBSS_192DataCreator creator;

        public static BBSS_192DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new BBSS_192DataCreator();
        ...
        {
            this.exerciseTitle = "875倍速算法练习";
            this.examTitle = "875倍速算法测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.BBSS_192.BBSS_192_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                A2 = (rand.Next(minValue, maxValue + 1) / 8) * 8; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A1 = 875;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
                //A2 = (decimal)System.Math.Pow(5, n); //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                //A1 = (decimal)System.Math.Pow(2, n); ;// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
                A1 = (rand.Next(minValue, maxValue + 1) / 8) * 8; //1000 * d1 + 100 * a1 + 10 * b1 + c1;
                A2 = 875;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
