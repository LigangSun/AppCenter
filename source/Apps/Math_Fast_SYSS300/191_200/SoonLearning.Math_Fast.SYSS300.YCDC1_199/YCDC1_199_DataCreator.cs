...
{
    public class YCDC1_199DataCreator : DataCreator
    {
        private static YCDC1_199DataCreator creator;

        public static YCDC1_199DataCreator Instance
        {
            get
            {
                if (creator == null)
                    creator = new YCDC1_199DataCreator();
        ...
        {
            this.exerciseTitle = "以乘代除法（一）练习";
            this.examTitle = "以乘代除法（一）测验";
            this.flowDocumentFile = "SoonLearning.Math_Fast.SYSS300.YCDC1_199.YCDC1_199_Document.xaml";
        ...
        private void GetRandomValues(SectionBaseInfo info, ref valuesStruct valueABC)
        {
            ...
            if (posRand == 0)
            {
                ...
                A2 = 5;// *(rand.Next(minValue + 1, maxValue + 1) / 5);// (decimal)System.Math.Pow(5, n);// 1000 * d2 + 100 * a2 + 10 * b2 + c2;
            }
            else
            {
